...
export function RegionalTeam(): React.JSX.Element {
    ...
    const [columns, setColumns] = useState({all: true, id: true, name: true, date: true, tg: true, vk: true, color: true, events: true})
    ...
    const handleAllChange = () => {
        setColumns(prevState => {
            ...
            const newColumns = {
                all: newAll, id: newAll, name: newAll, date: newAll, tg: newAll, vk: newAll, color: newAll, events: newAll
            };
                ...
                {isOpenNew &&
                    <div className={"absolute rounded-lg flex flex-col md:justify-center gap-5 z-50 w-full h-4/5 left-0 bottom-0 md:top-1/2 md:left-1/2 bg-white md:w-[500px] md:h-[200px] md:transform md:-translate-x-1/2 md:-translate-y-1/2 p-5"}>
                        ...
                        <div className={"flex flex-col gap-3"}>
                            ...
                            <input placeholder={"ID"} className={"rounded-lg border-[#3B64B3] border-2 py-1 px-2"}/>
                            <button className={"w-2/4 p-2 text-white bg-[#31AA27] rounded-lg self-center"}>Добавить</button>
                        </div>
                    </div>
                }
                        ...
                        {data.people && data.people.map(person =>
                            <tr className={cn("h-[50px] border-b-[1px]", styles.regionalTeam__tableBody)}>
                                ...
                                {columns.name && <th className={cn("sticky z-10 bg-white border-r-[1px] border-b-[1px]", !columns.id ? "left-0" : "left-[48px]")}>
                                    <input value={person.name} className={cn("border-0 h-10 bg-white w-full px-1 text-center", isEditorMode && "border-[1px]" )} disabled={!isEditorMode}/>
                                </th>}
                                {columns.date && <th>
                                    <input value={person.date} className={cn("border-0 h-10 bg-white w-full px-1 text-center", isEditorMode && "border-[1px]" )} disabled={!isEditorMode}/>
                                </th>}
                                {columns.tg && <th>
                                    <input value={person.tg} className={cn("border-0 h-10 bg-white w-full px-1 text-center", isEditorMode && "border-[1px]" )} disabled={!isEditorMode}/>
                                </th>}
                                {columns.vk && <th>
                                    <input value={person.vk} className={cn("border-0 h-10 bg-white w-full px-1 text-center", isEditorMode && "border-[1px]" )} disabled={!isEditorMode}/>
                                </th>}
                                ...
                                <th><button className={cn("bg-[#4471C9] text-white", isEditorMode && "cursor-no-drop bg-[#ABABAB]", styles.regionalTeam__newPassword)} disabled={!isEditorMode}>Выдать новый логин/пароль</button></th>
                                {isEditorMode &&
                                    <th className={cn("min-w-8 ")}><button className={"w-full"}><img src={bin} alt="delete"/></button></th>
                                }
                            </tr>
                        )}
                        </tbody>
